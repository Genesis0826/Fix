...
export class OnboardingService {
  ...
  async getMySession(accountId: string, sessionId?: string) {
    ...
    // Get the session
    const sessionQuery = supabase.from('onboarding_sessions').select('*');

    const { data: session, error: sessionErr } = sessionId
      ? await sessionQuery.eq('session_id', sessionId).limit(1).maybeSingle()
      : await sessionQuery
    ...
    let applicantProfile: { first_name: string; last_name: string } | null =
      null;
    if (!user) {
    ...
    for (const item of items || []) {
      ...
      const itemSubmissions = submissions.filter(
        (s) => s.onboarding_item_id === item.onboarding_item_id,
      );
      ...
      if (category === 'documents') {
        ...
        grouped.equipment.push({
          ...
          proof_of_receipt: itemSubmissions.filter(
            (s) => s.is_proof_of_receipt,
          ),
        });
  ...
  async uploadDocument(
    ...
    if (!allowedTypes.includes(file.mimetype)) {
      throw new BadRequestException(
        'Invalid file type. Only PDF, JPG, and PNG allowed.',
      );
    }
  ...
  async saveProfile(sessionId: string, dto: SaveProfileDto) {
    ...
    const payload = {
      ...
      emergency_phone_number:
        firstContact?.emergency_phone_number ?? emergency_phone_number ?? '',
      emergency_email_address:
        firstContact?.emergency_email_address ??
        emergency_email_address ??
        null,
    };
    ...
    if (profileItems && profileItems.length > 0) {
      await supabase
        ...
        .in(
          'onboarding_item_id',
          profileItems.map((i) => i.onboarding_item_id),
        );
    }
    ...
    if (ctx) {
      this.auditService
        .log(`PROFILE_SAVE: session ${sessionId}`, ctx.accountId, ctx.companyId)
        .catch(() => {});
    }
  ...
  async submitForReview(sessionId: string) {
    ...
    if (ctx) {
      ...
      this.notificationsService
        .notifyAllHRInCompany(ctx.companyId, {
    ...
    return {
      message: 'Onboarding submitted for HR review',
      session_id: sessionId,
      status: 'for-review',
    };
  }
  ...
  async updateItemStatus(
    ...
    const { data: item, error } = await supabase
      .from('onboarding_items')
      .select(
        'onboarding_item_id, session_id, template_item_id, template_items(title, tab_category)',
      )
      .eq('onboarding_item_id', onboardingItemId)
      .single();
    ...
    if (dto.status === 'approved' || dto.status === 'rejected') {
      ...
      if (ctx) {
        const itemTitle =
          (item as any).template_items?.title ?? 'Onboarding item';
        this.auditService
          .log(
        ...
        this.notificationsService
          .createNotification({
            ...
            title:
              dto.status === 'approved' ? 'Item Approved' : 'Item Rejected',
        ...
        if (ctx.employeeEmail) {
          this.mailService
            .sendOnboardingItemReviewedEmail({
    ...
    return {
      message: `Item marked as ${dto.status}`,
      onboarding_item_id: onboardingItemId,
      status: dto.status,
    };
  }
  ...
  async approveSession(sessionId: string, hrUserId?: string) {
    ...
    // === 100% completion gate ===
    // Count all required items and check they are all approved/confirmed
    ...
    if (itemsErr) {
      this.logger.warn(
        `[approveSession] Could not load items to validate completion: ${itemsErr.message}`,
      );
    } else {
      const requiredItems = (allItems ?? []).filter(
        (i: any) => i.template_items?.is_required === true,
      );
      const incompleteRequired = requiredItems.filter(
        (i: any) => !['approved', 'confirmed'].includes(i.status),
      );

      if (incompleteRequired.length > 0) {
        throw new BadRequestException(
          `Cannot approve: ${incompleteRequired.length} required onboarding item(s) are not yet completed. ` +
            `All required items must be approved or confirmed before the session can be approved.`,
        );
      }
    }
        ...
        try {
          const [{ data: applicant }, { data: staging }] = await Promise.all([
            ...
            supabase
              .from('employee_staging')
              .select(
                'first_name, last_name, email_address, complete_address, date_of_birth, place_of_birth, nationality, civil_status',
              )
              .eq('session_id', sessionId)
              .maybeSingle(),
          ]);
          ...
          if (applicant) {
            ...
            await supabase.from('user_profile').insert({
              ...
              ...(staging?.complete_address
                ? { complete_address: staging.complete_address }
                : {}),
              ...(staging?.date_of_birth
                ? { date_of_birth: staging.date_of_birth }
                : {}),
              ...(staging?.place_of_birth
                ? { place_of_birth: staging.place_of_birth }
                : {}),
              ...(staging?.nationality
                ? { nationality: staging.nationality }
                : {}),
              ...(staging?.civil_status
                ? { civil_status: staging.civil_status }
                : {}),
            });
            ...
            try {
              ...
              this.logger.log(
                `[approveSession] invite link for ${loginEmail}: ${inviteLink}`,
              );
            }
          }
        } catch (provisionErr) {
          this.logger.error(
            `[approveSession] Failed to provision employee account: ${provisionErr?.message}`,
          );
        }
      }
    }
    ...
    try {
      if (resolvedUserId) {
        ...
        const { data: sessionItems } = await supabase
          .from('onboarding_items')
          .select(
            'onboarding_item_id, template_items!inner(tab_category, title)',
          )
          .eq('session_id', sessionId)
          ...
          if (docs && docs.length > 0) {
            ...
            for (const doc of docs) {
              const item = sessionItems.find(
                (i: any) => i.onboarding_item_id === doc.onboarding_item_id,
              ) as any;
              const docType =
                item?.template_items?.title || 'onboarding-document';
            ...
            await supabase.from('employee_documents').upsert(docRows, {
      ...
      this.logger.error(
        `[approveSession] Failed to sync profile/docs: ${syncErr?.message}`,
      );
    }

    // === Compensation & Benefits handoff event ===
    // Fire-and-forget: emit an audit event and in-app notification to HR officers
    // signalling that the new employee is ready for C&B enrollment.
    try {
      const ctx = await this.getSessionContext(sessionId);
      if (ctx?.companyId) {
        await this.auditService.log(
          `COMPENSATION_BENEFITS_HANDOFF: session ${sessionId} – new employee ready for C&B enrollment`,
          hrUserId ?? ctx.accountId,
          ctx.companyId,
        );
        ...
        await this.notificationsService.notifyAllHRInCompany(ctx.companyId, {
          ...
          title: 'New Employee Ready for C&B Enrollment',
          message: `${ctx.employeeName || 'A new employee'} has completed onboarding and is ready for Compensation & Benefits enrollment.`,
          metadata: {
            session_id: sessionId,
            employee_name: ctx.employeeName,
            employee_email: ctx.employeeEmail,
          },
        });
      }
    } catch (cbErr) {
      this.logger.error(
        `[approveSession] Failed to emit C&B handoff: ${cbErr?.message}`,
      );
    }
  ...
  async createTemplate(dto: CreateTemplateDto) {
    ...
    if (!position)
      throw new BadRequestException(
        'Invalid position_id: position does not exist',
      );
    ...
    if (!dept)
      throw new BadRequestException(
        'Invalid department_id: department does not exist',
      );
  ...
  async assignTemplate(dto: AssignTemplateDto) {
    ...
    this.logger.log(
      `Template ${dto.template_id} assigned to ${dto.account_id}`,
    );
  ...
  async updateTemplateItem(
    itemId: string,
    dto: { title?: string; description?: string; is_required?: boolean },
  ) {
    const supabase = this.supabaseService.getClient();
  ...
  private async recalculateProgress(sessionId: string) {
    ...
    const trackable = items.filter(
      (i: any) => i.template_items.tab_category !== 'welcome',
    );
  ...
  async saveOnboarding(applicantId: string, body: Record<string, any>) {
    ...
    if (submission.status === 'submitted')
      throw new BadRequestException(
        'Onboarding is under review. Wait for HR feedback before editing.',
      );
  ...
  async submitOnboarding(applicantId: string) {
    ...
    if (submission.status === 'submitted')
      throw new BadRequestException(
        'Onboarding is already submitted and under review.',
      );
  ...
  async getHROnboardingSubmissions(companyId: string, statusFilter?: string) {
    ...
    let query = supabase
      .from('onboarding_submissions')
      .select(
        '*, applicant_profile ( first_name, last_name, email, phone_number ), job_postings ( title, company_id )',
      )
    ...
    let fallbackQuery = supabase
      .from('onboarding_submissions')
      .select(
        '*, applicant_profile ( first_name, last_name, email, phone_number ), job_postings!inner ( title, company_id )',
      )
    ...
    const mismatched = (fallbackData ?? []).filter(
      (s: any) => s.company_id !== companyId,
    );
    if (mismatched.length > 0) {
      await supabase
        ...
        .in(
          'submission_id',
          mismatched.map((s: any) => s.submission_id),
        );
    }
  ...
  async getHROnboardingSubmission(submissionId: string, companyId: string) {
    ...
    const { data, error } = await supabase
      .from('onboarding_submissions')
      .select(
        '*, applicant_profile ( first_name, last_name, email, phone_number ), job_postings ( title )',
      )
  ...
  async approveOnboardingSubmission(
    ...
    if (submission.status !== 'submitted')
      throw new BadRequestException(
        'Submission must be in "submitted" state to approve.',
      );
    if (!submission.preferred_username)
      throw new BadRequestException(
        'Applicant must provide a preferred username before approval.',
      );
    ...
    try {
      ...
      this.logger.error(
        `Failed to seed employee_documents: ${seedErr?.message}`,
      );
    }
    ...
    try {
      ...
      this.logger.log(
        `[onboarding approve] invite link for ${applicant?.email}: ${inviteLink}`,
      );
    }
  ...
  async rejectOnboardingSubmission(
    ...
    if (submission.status !== 'submitted')
      throw new BadRequestException(
        'Only submitted onboarding forms can be rejected.',
      );
    const { error } = await supabase
      .from('onboarding_submissions')
  ...
  async createApplicantSession(params: {
    ...
    if (!templateId) {
      this.logger.error(
        'No onboarding template found — cannot create applicant session',
      );
      return null;
    }
    ...
    if (sessionErr) {
      this.logger.error(
        `Failed to create applicant session: ${sessionErr.message}`,
      );
    ...
    if (templateItems.length > 0) {
      ...
      if (itemsErr) {
        this.logger.error(
          `Failed to create onboarding items for applicant session: ${itemsErr.message}`,
        );
      }
    }

    this.logger.log(
      `Applicant onboarding session created: ${sessionId} for applicant ${params.applicantId}`,
    );
